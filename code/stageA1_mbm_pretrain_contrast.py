...
from config import Config_MBM_fMRI_contrast
from dataset import hcp_dataset
from sc_mbm.mae_for_fmri import MAEforFMRIContrast
...
def get_args_parser():
    ...
    parser.add_argument('--do_self_contrast', type=bool, default=False)
    parser.add_argument('--do_mask_loss', type=bool, default=False)
    parser.add_argument('--use_target_as_pos', type=bool, default=False)
    parser.add_argument('--contrast_loss_weight', type=float)
    parser.add_argument('--mask_loss_weight', type=float)

    parser.add_argument('--do_cross_contrast', type=bool)
    parser.add_argument('--cross_contrast_loss_weight', type=float)
...
def plot_recon_figures(model, device, dataset, output_path, num_figures = 5, config=None, logger=None, model_without_ddp=None):
    ...
    for ax in axs:
        ...
        loss, pred, mask, _, _, _ = model(sample, mask_ratio=config.mask_ratio)
...
if __name__ == '__main__':
    ...
    config = Config_MBM_fMRI_contrast()
