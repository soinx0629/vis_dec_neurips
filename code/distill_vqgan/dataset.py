...
def create_BOLD5000_dataset(path='data/BOLD5000', patch_size=16, fmri_transform=identity,
            image_transform=identity, subjects = ['CSI1', 'CSI2', 'CSI3', 'CSI4'], include_nonavg_test=True, num_voxels=4192, do_normalize=False):
    # 一次只用一个被试， do_normalize=True
    ...
    for sub in subjects:
        ...
        # 按index取出图片文件名
        test_img_name = [img_files[idx[0]] for idx in test_idx]
   ...
   # num_voxels = fmri_train_major.shape[-1]
    if isinstance(image_transform, list):
        return (BOLD5000_dataset(fmri_train_major, img_train_major, img_name_train_major, fmri_transform, image_transform[0], num_voxels),
                BOLD5000_dataset(fmri_test_major, img_test_major, img_name_train_major, torch.FloatTensor, image_transform[1], num_voxels))
    else:
        return (BOLD5000_dataset(fmri_train_major, img_train_major, img_name_test_major, fmri_transform, image_transform, num_voxels),
                BOLD5000_dataset(fmri_test_major, img_test_major, img_name_test_major, torch.FloatTensor, image_transform, num_voxels))
...
class BOLD5000_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        fmri = np.pad(fmri, ((0,0), (0, self.num_voxels-fmri.shape[-1])), 'wrap') 
