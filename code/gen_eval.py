...
import datetime
import argparse
from distill_vqgan.main import get_distillation_model
...
if __name__ == '__main__':
    ...
    if target == 'GOD':
        ...
        _, dataset_test = create_BOLD5000_dataset(config.bold5000_path, config.patch_size, 
                fmri_transform=torch.FloatTensor, image_transform=img_transform_test, 
                subjects="CSI4")#config.bold5000_subs)
    else:
    ...
    distillation_model = get_distillation_model("logs/2023-02-06T15-31-10/testtube/version_None/checkpoints/epoch=134-step=96120.ckpt")
    print(len(dataset_test))

    grid, samples = generative_model.generate(dataset_test, config.num_samples, 
                config.ddim_steps, config.HW, limit=None, state=state,
                vqgan_model=distillation_model) # generate 10 instances
